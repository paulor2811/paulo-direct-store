      ...
      <div class="lg:grid lg:grid-cols-2 lg:gap-8 xl:gap-16">
        ...
        <div class="mt-6 sm:mt-8 lg:mt-0">
          ...
          <a href="{{ $vendorUrl }}" class="mt-6 flex items-center p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-primary-500/30 hover:bg-white dark:hover:bg-gray-800 transition-all group">
              <div class="w-12 h-12 rounded-xl bg-white dark:bg-gray-800 flex items-center justify-center shadow-sm text-primary-600 mr-4 overflow-hidden">
                  @if($product->store_id && $product->store)
                    @if($product->store->logo_url)
                        <img src="{{ $product->store->logo_url }}" alt="{{ $product->store->nome }}" class="w-full h-full object-cover">
                    @else
                  ...
                  @else
                    @if($product->user && $product->user->profile_photo_url)
                        <img src="{{ $product->user->profile_photo_url }}" alt="{{ $product->user->name }}" class="w-full h-full object-cover">
                    @else
              ...
              <div class="flex-1">
                  <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Vendido por</p>
                  <span class="block text-lg font-black text-gray-900 dark:text-white group-hover:text-primary-600 transition-colors line-clamp-1">
                      @if($product->store_id && $product->store)
                        {{ $product->store->nome }}
                      @else
                        {{ $product->user->name ?? 'Vendedor Independente' }}
                      @endif
                  </span>
              </div>
          ...
          <div class="mt-6 sm:gap-4 sm:items-center sm:flex sm:mt-8">
            ...
            <a
              ...
              class="text-white mt-4 sm:mt-0 bg-[#25D366] hover:bg-[#20bd5a] focus:ring-4 focus:ring-[#25D366]/50 font-black rounded-xl text-xs px-8 py-4 flex items-center justify-center w-full sm:w-auto transition-all active:scale-95 uppercase tracking-widest shadow-lg shadow-emerald-500/20"
              role="button"
            >
              <svg class="w-5 h-5 mr-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
